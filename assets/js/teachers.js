    ...
    async loadTeachers() {
        try {
            ...
            if (window.dataLoader) {
                ...
                console.log('✅ Successfully loaded teachers from JSON:', this.teachers.length);
                console.log('✅ Sample teacher data:', this.teachers[0]);
                ...
                console.log('❌ DataLoader not available - loading directly');
                // Direct fetch as fallback
                const response = await fetch('data/teachers.json');
                if (response.ok) {
                    this.teachers = await response.json();
                    this.filteredTeachers = [...this.teachers];
                    console.log('✅ Successfully loaded teachers from direct fetch:', this.teachers.length);
                } else {
